 ...
 * Return: 0 - tree is NULL, else - height
 */
size_t height(const binary_tree_t *tree)
{
	if (tree != NULL)
 ...
 * Return: 0 - tree is NULL, else - balance factor
 */
int balance(const binary_tree_t *tree)
{
 ...
 * Return: pointer to the new root after insertion, NULL - failure
 */
avl_t *avl_insert_recursive(avl_t **tree, avl_t *parent,
		avl_t **new, int value)
{
 ...
 * @tree: double ptr to the root node of the AVL tree to insert into
 * @value: value to insert into the AVL tree
 * Return: ptr to the inserted node, NULL - failure
 */
avl_t *avl_insert(avl_t **tree, int value)
{
	avl_t *new = NULL;
